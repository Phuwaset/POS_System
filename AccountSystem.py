...
#ใช้ Tkinter สำหรับการสร้างหน้าจอ UI.
#ใช้ PIL สำหรับจัดการภาพ.
#ใช้ sqlite3 สำหรับจัดการฐานข้อมูลที่เก็บข้อมูลผู้ใช้.
...
#โค้ดนี้เป็นโปรแกรม GUI ที่ใช้ไลบรารี Tkinter สำหรับสร้างระบบหน้าล็อกอิน, สมัครสมาชิก, และหน้าการจัดการบัญชี (Account Management System)
#ของร้านกาแฟ (GIDEONS COFFEE SHOP) ระบบนี้รองรับผู้ใช้สามประเภทคือ Admin, Guest และ Employee 
# โดยแต่ละคนจะมีหน้าเฉพาะสำหรับการจัดการข้อมูลส่วนตัว
...
class AccountPage:
    def __init__(self, AccountSystem_window):
        ...
        AccountSystem_window.title('ScaleUp LedgerX')
...
# -----------------------------------------------------------------------------------------
        ...
        # win = Toplevel(): สร้างหน้าต่างใหม่ (Toplevel window) ที่จะแยกออกมาจากหน้าต่างหลัก.
        # CoffeeManagementSystem.Guest.GuestPage(win): เรียกใช้หน้าจอสำหรับ Guest โดยใช้คลาส GuestPage ที่อยู่ในโมดูล Guest.
        # AccountSystem_window.withdraw(): ซ่อนหน้าต่างปัจจุบันเพื่อให้ผู้ใช้โฟกัสที่หน้าต่างใหม่.
        # win.deiconify(): แสดงหน้าต่างใหม่ที่สร้างขึ้นมา.
...
#       ลักษณะการทำงานคล้ายกับ open_guest() แต่ในกรณีนี้ ฟังก์ชันจะเรียกใช้หน้า FirstPage ของ Employee จากโมดูล Dashboard (ที่น่าจะเป็นหน้าสำหรับการจัดการหรือทำงานของพนักงาน).
#       นอกจากนี้ หน้าต่างใหม่จะถูกแสดงและหน้าต่างปัจจุบันจะถูกซ่อน.
        ...
        def login_all():
            ...
            # คำสั่ง SQL นี้ใช้สำหรับค้นหาข้อมูลจากตาราง Admin_Account โดยตรวจสอบว่าชื่อผู้ใช้ (admin_username) และรหัสผ่าน (admin_password) ตรงกับค่าที่ผู้ใช้กรอกเข้ามาหรือไม่.
            # สัญลักษณ์ ? ใน SQL เป็นตัวแทนของข้อมูลที่จะถูกส่งเข้ามา (เพื่อความปลอดภัยในการป้องกัน SQL Injection).
        ...
        # หน้าจอหลัก (AccountPage Class):
        # ในคลาส AccountPage มีการสร้างหน้าจอ UI หลักสำหรับการล็อกอินและสมัครสมาชิก โดยหน้าจอจะประกอบไปด้วย สามส่วนหลัก คือ:
        # หน้า Landing Page สำหรับแสดงข้อมูลเกี่ยวกับร้านและปุ่มสำหรับเลือก Login หรือ Create New Account.
        # หน้า Sign In Page สำหรับล็อกอินเข้าสู่ระบบ.
        # หน้า Sign Up Page สำหรับสร้างบัญชีผู้ใช้ใหม่.
